...
def write(d, num):
    ...
    for i in range(5, 6+1):
        with open(f'{alg[i]}.txt', 'a') as txt:
...
for num_of_elem in range(50000000, 100000000+1, 10000000):
    data = [randint(1, 999999999) for i in range(num_of_elem)]
    # a = data.copy()
    # b = data.copy()
    # c = data.copy()
    # d = data.copy()
    ...
    if num_of_elem % 10000 == 0:
        ...
        # plt.savefig(f'all_sort{num_of_elem}.png')
        # plt.close()

        plt.plot(dict1[5][0], dict1[5][1])
        plt.plot(dict1[6][0], dict1[6][1])
        plt.legend(('quick_sort', 'comb_sort'))
        plt.savefig(f'2_sort{num_of_elem}.png')
        plt.close()
